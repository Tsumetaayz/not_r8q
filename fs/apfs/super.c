...
static int apfs_check_features(struct super_block *sb)
{
	...
	if (features & (APFS_INCOMPAT_CASE_INSENSITIVE | APFS_INCOMPAT_NORMALIZATION_INSENSITIVE))
		sbi->use_hash = 1;
	else
		sbi->use_hash = 0;
	apfs_info(sb, "mounting a case-%ssensitive filesystem", sbi->use_hash ? "in" : "");
...
{
	const char *sep = strchr(dev_name, '+');
	if(sep) {
		char *aname, *adata;
		aname = kzalloc(sep - dev_name + 1, GFP_KERNEL);
		if(!aname)
			return ERR_PTR(-ENOMEM);
		memcpy(aname, dev_name, sep - dev_name);
		if(data) {
			adata = kzalloc(strlen(data) + strlen(sep + 1) + 6, GFP_KERNEL);
			if(!adata) {
				kfree(aname);
				return ERR_PTR(-ENOMEM);
			}
			sprintf(adata, "%s,vol=%s", (const char *)data, sep + 1);
		} else {
			adata = kzalloc(strlen(sep + 1) + 5, GFP_KERNEL);
			if(!adata) {
				kfree(aname);
				return ERR_PTR(-ENOMEM);
			}
			sprintf(adata, "vol=%s", sep + 1);
		}
		/* yeah this leaks memory. :( */
		return mount_bdev(fs_type, flags | SB_MULTIMOUNT, aname, adata, apfs_fill_super);
	}
